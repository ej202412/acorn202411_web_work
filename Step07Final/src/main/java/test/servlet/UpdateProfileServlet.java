...
public class UpdateProfileServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(filePart != null && filePart.getSize() > 0) {
			...
			UserDao.getInstance().updateEmailProfile(dto); //이메일, 프로필 수정
		}else {//선택한 프로필 이미지가 없는 경우 
			//여기서는 프로필 이미지가 수정되면 안된다.
			UserDao.getInstance().updateEmail(dto); //이메일 수정
		}
		
		String cPath=req.getContextPath();
		resp.sendRedirect(cPath+"/user/protected/info.jsp"); //개인정보 보기로 리다이렉트 응답
	}
}
